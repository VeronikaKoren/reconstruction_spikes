...
task=['plot LD layer ',namep{period}];
display(task)

savefile='/home/veronika/Dropbox/reconstruction/figures/layer/';
figname=['lay_sess_',namep{period}];
pos_vec=[0,0,14,12];
...
for ba=1:2
    ...
    xnm_all{1,ba}=squeeze(x_sg(:,1,:));                                                 % condition nm
    xnm_all{2,ba}=squeeze(x_g(:,1,:));
    xnm_all{3,ba}=squeeze(x_ig(:,1,:));
    
    xm_all{1,ba}=squeeze(x_sg(:,2,:));                                                 % condition nm
    xm_all{2,ba}=squeeze(x_g(:,2,:));
    xm_all{3,ba}=squeeze(x_ig(:,2,:));
...
for ba=1:2
    for r=1:3
        ...
        y1=mnm{r,ba}-snm{r,ba}./sqrt(nbses);
        y2=mnm{r,ba}+snm{r,ba}./sqrt(nbses);
        patch([x fliplr(x)], [y1 fliplr(y2)], blue,'FaceAlpha',0.3,'EdgeColor',blue)
        
        z1=mm{r,ba}-sm{r,ba}./sqrt(nbses);
        z2=mm{r,ba}+sm{r,ba}./sqrt(nbses);
        ...
        if ba==1
            if r==1&&ba==1
                text(0.12,0.78,namebeh{1},'units','normalized','color',col(1,:),'FontName','Arial','fontsize',fs)
                text(0.12,0.9,namebeh{2},'units','normalized','color',col(2,:),'FontName','Arial','fontsize',fs)
            end
        end
